...
export default class Detector {
  ...
  detect(targetWidth: number, targetHeight: number) {
    ...
    this.classifier.detectMultiScale(
      ...
      4,
      0,
      new cv.Size(FACE_MIN_WIDTH, FACE_MIN_WIDTH)
    );
    if (this.faces.get(0) && this.faces.get(0).width >= FACE_MIN_WIDTH) {
      // this can be encoded into detect multiscale size param
      ...
      cv.resize(
        crop,
        crop,
        new cv.Size(targetHeight, targetWidth),
        0,
        0,
        cv.INTER_AREA
      );
